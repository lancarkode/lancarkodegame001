# Lancarkode game #001
# Title     : Shoot Factor!!! - V17
# Author    : Patuan P. T.
# Date      : 11/05/21

#################
# In this version, I add the winner or lose event
...
import random  # this is the way we import random
...
RESTART_EVENT = pygame.USEREVENT + 4
...
# Text style
TEXT_STYLE_ONE = pygame.font.SysFont("comicsans", 30)
TEXT_STYLE_TWO = pygame.font.SysFont("comicsans", 100)

# Bullet point box
BULLET_BOX_X = 0
BULLET_BOX_Y = PLANET.height
BULLET_BOX_WIDTH = PLANET.width
BULLET_BOX_HEIGHT = HEIGHT - PLANET.height
BULLET_BOX = pygame.Rect(BULLET_BOX_X, BULLET_BOX_Y, BULLET_BOX_WIDTH, BULLET_BOX_HEIGHT)
BULLET_BOX_TITLE = TEXT_STYLE_ONE.render("Bullet Point", True, WHITE)

# Hardness point box
HARDNESS_BOX_X = WIDTH - BULLET_BOX_WIDTH
HARDNESS_BOX_Y = PLANET.height
HARDNESS_BOX_WIDTH = PLANET.width
HARDNESS_BOX_HEIGHT = HEIGHT - PLANET.height
HARDNESS_BOX = pygame.Rect(HARDNESS_BOX_X, HARDNESS_BOX_Y, HARDNESS_BOX_WIDTH, HARDNESS_BOX_HEIGHT)
HARDNESS_BOX_TITLE = TEXT_STYLE_ONE.render("Hardness Point", True, WHITE)

# We add the space background here
SPACE_X = PLANET.width
SPACE_Y = 0
SPACE_WIDTH = WIDTH - PLANET.width
SPACE_HEIGHT = PLANET.height
...
def screen_draw(character, asteroid_one, asteroid_two, asteroid_three, bullet_list,
                ...
                bullet_point, aim_rect, play_result):
    # we fill the screen with white color
    SCREEN.fill(WHITE)
    # drawing the planet, WE CHANGE IT TO BE GREEN
    pygame.draw.rect(SCREEN, GREEN, PLANET)
    # drawing the space
    SCREEN.blit(SPACE_IMG, (SPACE_X, SPACE_Y), (SPACE_X, SPACE_Y, SPACE_WIDTH, SPACE_HEIGHT))
    # bullet point box display
    pygame.draw.rect(SCREEN, ORANGE, BULLET_BOX)  # drawing the bullet box
    SCREEN.blit(BULLET_BOX_TITLE, (BULLET_BOX_WIDTH//5, BULLET_BOX_Y+10))  # write the bullet box title
    bullet_point_text = str(bullet_point)  # convert int to string
    bullet_point_text_render = TEXT_STYLE_TWO.render(bullet_point_text, True, WHITE)  # render the text
    SCREEN.blit(bullet_point_text_render, (BULLET_BOX_X+BULLET_BOX_WIDTH//3, BULLET_BOX_Y+50))  # Show the Bullet point
    # hardness point box display
    pygame.draw.rect(SCREEN, ORANGE, HARDNESS_BOX)  # draw the hardness point box
    SCREEN.blit(HARDNESS_BOX_TITLE, (HARDNESS_BOX_X+50, HARDNESS_BOX_Y+10))  # write the hardness box title
    hardness_point_text = " "
    if aim_rect.colliderect(asteroid_one) and asteroid_one_hardness > MIN_HARDNESS:
        hardness_point_text = str(asteroid_one_hardness)
    elif aim_rect.colliderect(asteroid_two) and asteroid_two_hardness > MIN_HARDNESS:
        hardness_point_text = str(asteroid_two_hardness)
    elif aim_rect.colliderect(asteroid_three) and asteroid_three_hardness > MIN_HARDNESS:
        hardness_point_text = str(asteroid_three_hardness)
    hardness_point_text_render = TEXT_STYLE_TWO.render(hardness_point_text, True, WHITE)
    SCREEN.blit(hardness_point_text_render, (HARDNESS_BOX_X+HARDNESS_BOX_WIDTH//3, HARDNESS_BOX_Y+50))
    # if you want to draw the aim rect, then uncomment the below
    # pygame.draw.rect(SCREEN, BLACK, aim_rect)
    # we blit the main character after the screen
    ...
    for one_bullet in bullet_list:
        ...
        # win or lost display
    if play_result != "":
        play_again_text = "Press 'R' to Restart the game."
        play_result_text = TEXT_STYLE_ONE.render(play_result, True, BLACK)
        play_again = TEXT_STYLE_ONE.render(play_again_text, True, BLACK)
        SCREEN.blit(play_result_text, (BULLET_BOX_WIDTH + BULLET_BOX_WIDTH * 0.45, BULLET_BOX_Y))
        SCREEN.blit(play_again, (BULLET_BOX_WIDTH + BULLET_BOX_WIDTH * 0.45 - 50, BULLET_BOX_Y + 50))
    pygame.display.update()  # we update the display every time


# Move the character
# The character can move forward, backward, upward, dan downward
def character_move(character_object, aim_rect, key_press):
...
def main():
    ...
    # we add the aim rect here
    aim_rect = pygame.Rect(CHARACTER_INIT_X+CHARACTER_WIDTH, CHARACTER_INIT_Y+CHARACTER_HEIGHT//2, WIDTH, 5)
    ...
    number = 3
    hardness = random.sample(range(start, end), number)
    asteroid_one_hardness = hardness[0]
    asteroid_two_hardness = hardness[1]
    asteroid_three_hardness = hardness[2]
    ...
    while run:  # if run is True, then the game window appear and running until QUIT event happen
        ...
        for event in pygame.event.get():  # listening to every game event and creating all event
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # Key pressed for restarting the game
                if event.key == pygame.K_r:
                    pass

            # create event when the asteroids are hit
            # we change the value to be bullet_score variable
            if event.type == ASTEROIDS_ONE_EVENT:
                asteroid_one_hardness -= bullet_point
            if event.type == ASTEROIDS_TWO_EVENT:
                asteroid_two_hardness -= bullet_point
            if event.type == ASTEROIDS_THREE_EVENT:
                asteroid_three_hardness -= bullet_point

        # Win or Lose
        if asteroid_one.colliderect(PLANET) or asteroid_two.colliderect(PLANET) or \
                asteroid_three.colliderect(PLANET):
            play_result = "You are lost!!!"
        elif asteroid_one_hardness <= MIN_HARDNESS and \
                asteroid_two_hardness <= MIN_HARDNESS and \
                asteroid_three_hardness <= MIN_HARDNESS:
            play_result = "You are the WINNER!!!!"
        else:
            # Key get pressed function
            key_press = pygame.key.get_pressed()

            # we move the character. Now we input the key_press inside the character_move()
            # we let the aim_rect also move with the main character
            character_move(character, aim_rect, key_press)
        ...
        # we add the bullet point text to the screen draw
        # we draw the aim_rect first, after finish we have to remove from the drawing
