# Lancarkode game #001
# Title     : Shoot Factor!!! - V8
# Author    : Patuan P. T.
# Date      : 11/04/21

#################
# In this version, we control the movement of the character
# We will use the keyboard press function in the pygame
# please pay attention the while run and character_move()
#################
...
def character_move(character_object, key_press):
    if key_press[pygame.K_RIGHT]:  # moving forward if the right arrow key is pressed
        character_object.x += CHARACTER_VEL
    if key_press[pygame.K_LEFT]:  # moving backward if the left arrow key is pressed
        character_object.x -= CHARACTER_VEL
    if key_press[pygame.K_UP]:  # moving upward if the up arrow key is pressed
        character_object.y -= CHARACTER_VEL
    if key_press[pygame.K_DOWN]:  # moving downward if the down arrow key is pressed
        character_object.y += CHARACTER_VEL
