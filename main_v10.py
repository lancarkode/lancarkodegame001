# Lancarkode game #001
# Title     : Shoot Factor!!! - V10
# Author    : Patuan P. T.
# Date      : 11/04/21

#################
# In this version, you will learn how to add an event in pygame.
# Here, the event is the main character can fire bullets
# The concept is that we draw rectangle each time a key is pressed.
# The information that the rectangle will have are:
# 1. x coordinate
# 2. y coordinate
# 3. width
# 4. height
# 5. velocity
# The player might be pressed the key fire very fast.
# We must put the bullet/rectangle in a list
#################
...
# bullet shoot
def bullet_shoot(bullet_list, vel):
    for one_bullet in bullet_list:
        one_bullet.x += vel
        # if the bullet is out side the screen we remove the bullet
        # for saving our RAM
        if one_bullet.x + vel > WIDTH:
...
def main():
    ...
    while run:  # if run is True, then the game window appear and running until QUIT event happen
        ...
        bullet_shoot(bullet_list, BULLET_VEL)
