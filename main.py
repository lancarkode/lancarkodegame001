# Lancarkode game #001
# Title     : Shoot Factor!!! - V16
# Author    : Patuan P. T.
# Date      : 12/04/21

#################
# In this version, we will let the player choose the bullet point
#################
...
def screen_draw(character, asteroid_one, asteroid_two, asteroid_three, bullet_list,
                ...
                bullet_point, aim_rect):
    # we fill the screen with white color
    SCREEN.fill(WHITE)
    # drawing the planet
    pygame.draw.rect(SCREEN, BLUE, PLANET)
...
# bullet shoot
def bullet_shoot(bullet_list, bullet_point_list, asteroid_one, asteroid_two, asteroid_three,
                 asteroid_one_event, asteroid_two_event, asteroid_three_event,
                 asteroid_one_hardness, asteroid_two_hardness, asteroid_three_hardness):

    for one_bullet in bullet_list:
        for one_point in bullet_point_list:
            ...
            if asteroid_one.colliderect(one_bullet) and asteroid_one_hardness > MIN_HARDNESS:
                ...
                bullet_list.remove(one_bullet)
                bullet_point_list.remove(one_point)
...
def main():
    ...
    # bullet score
    bullet_point = ""  # for displaying
    shoot_point = ""  # for calculation
    bullet_point_list = []
    ...
    while run:  # if run is True, then the game window appear and running until QUIT event happen
        ...
        for event in pygame.event.get():  # listening to every game event and creating all event
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_f and bullet_point != "" and shoot_point != "":
                    # create a bullet
                    bullet = pygame.Rect(character.x + character.width,
                                         character.y + character.height // 2, 10, 5)
                    # put the bullet in the list
                    bullet_list.append(bullet)
                    # put the bullet point in the list
                    bullet_point_list.append(shoot_point)
                    bullet_point = ""

                # event while user typing for the bullet point
            ...
            if event.type == ASTEROIDS_ONE_EVENT:
                asteroid_one_hardness -= int(shoot_point)

            if event.type == ASTEROIDS_TWO_EVENT:
                asteroid_two_hardness -= int(shoot_point)

            if event.type == ASTEROIDS_THREE_EVENT:
                asteroid_three_hardness -= int(shoot_point)
        ...
        bullet_shoot(bullet_list, bullet_point_list, asteroid_one, asteroid_two, asteroid_three,
